...
BluePrint::BluePrint(MySQL* msql, const std::string name) : 
_sql(msql), 
...
void BluePrint::addTag(std::string name, sqltypes sqltype)
{
    _tags.push_back(std::make_tuple(name, sqltype));
    doesStatementNeedBuilt = true;
    rebuildStatements();
}
